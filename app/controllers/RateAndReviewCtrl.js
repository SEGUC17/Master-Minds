  ...
  business.find({'business_name':req_business},function(err,found_business)
          {
            ...
            {
              console.log(401);
              res.status(401).send('no business found error happend in the Post_Rate_Business');
            }
            else
              { var obj_rate= found_business[0].business_rating;
                ...
                {
                  if(obj_rate[i].username==new_rate.username)
                  { ffound=1;
...
exports.Get_Review_Numbered_Service = function(req,res)
{
  ...
  var req_number =req.param('number_of_reviews_per_page');
  var req_page = req.param('page_number');
   business.find({'business_name':req_business,services:{'service_name':req_service }},function(err,found_service)
   {
       if(err)
       {
         console.log(401);
...
{ var businesses = require('mongoose').model('businesses');
  var req_business=req.param('business');
  var req_review=req.body.review;


  businessOwners.findOne({'business_name':req_business},function(err,found_business)
  {
        if(err)
       {
         console.log(401);
       ...
       else
       {
         ...
         if(!reviews||reviews[i])
         {
            ...
            res.status(404).send("review not found");
                 ...
                 for(var i=0;i<reviews.length;i++)
                {
                    if(reviews[i].review == req_review)
                    {console.log("here");
                      reviews[i].report++;
                      console.log(reviews[i].username+" "+reviews[i].report);
                      return;
                    }
                }
                console.log(200);
                res.status(200).send("you report is added");
               }
