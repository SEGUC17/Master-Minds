        ...
        UserLoginController.getUserByUsername(username, function(err, user) {
            ...
            if (!user) {
              ...
              adminLoginController.getAdminByUsername(username,function(err, admin){
                  ...
                  if (err) throw err;
                  if (!admin){
                    serviceController.getbusinessownerByEmail(personal_email,function(err,business_owner){
                      if(err) throw err;
                      if(!Business){
                        console.log("Reached here 2");
                        return done(null, false);
                      }
                      serviceController.comparePassword(password,business_owner.password,function(err,isMatch){
                        if(err) throw err;
                        if(isMatch){
                          return done (null,business_owner);
                        }else{
                          return done(null,false);
                        }
                      })
